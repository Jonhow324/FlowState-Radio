...
class Brain {
  ...
  _buildRhythmGuide() {
    return [
      ...
      '  - type: "bridge"（过渡串词，文字长短由你根据语境自由决定）',
      '  - type: "silence"（刻意留白，不说话，text 为空字符串）',
  ...
  _buildRAGPrompt(context, candidates) {
    ...
    return [
      ...
      '# 输出要求',
      '严格返回 JSON 格式：',
      '{"say": "DJ开场串词(自然语言，可为null)", "songs": [{"name": "歌曲名", "artist": "歌手", "transition_style": "intro|outro|none"}], "segments": [{"type": "cold_open|bridge|silence|back_announce", "text": "话术文本(silence时为空)", "anchor": 0, "position": "before_track|between_tracks|after_track"}], "reason": "选歌理由"}',
      '',
