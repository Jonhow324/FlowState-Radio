...
function ProfileView() {
  ...
  if (loading) {
    return (
      <div className="flex items-center justify-center py-20">
        <div className="w-6 h-6 border-2 border-flowstate-500 border-t-transparent rounded-full animate-spin" />
      </div>
    );
  }
  ...
  return (
    ...
    >
      ...
      >
        <h3 className="text-sm font-medium text-flowstate-400 mb-3">播放统计</h3>
        <div className="grid grid-cols-3 gap-3">
          <div className="text-center p-3 bg-white/5 rounded-xl">
            <p className="text-2xl font-bold text-flowstate-400">{stats?.today || 0}</p>
            <p className="text-xs text-white/40">今日</p>
          </div>
          <div className="text-center p-3 bg-white/5 rounded-xl">
            <p className="text-2xl font-bold text-flowstate-400">{stats?.week || 0}</p>
            <p className="text-xs text-white/40">本周</p>
          </div>
          <div className="text-center p-3 bg-white/5 rounded-xl">
            <p className="text-2xl font-bold text-flowstate-400">{stats?.total || 0}</p>
      ...
      {stats?.topArtists?.length > 0 && (
        <motion.div
          ...
          variants={cardVariants}
        >
          <h3 className="text-sm font-medium text-flowstate-400 mb-3">最爱歌手</h3>
          <div className="space-y-2">
            {stats.topArtists.map((a, i) => (
              <div key={a.artist} className="flex items-center justify-between">
                ...
                <span className="text-xs text-flowstate-500">{a.count} 次</span>
              </div>
            ))}
          </div>
        </motion.div>
      ...
      {stats?.topTracks?.length > 0 && (
        <motion.div
          ...
          variants={cardVariants}
        >
          <h3 className="text-sm font-medium text-flowstate-400 mb-3">最爱歌曲</h3>
          <div className="space-y-2">
            {stats.topTracks.map((t, i) => (
              <div key={`${t.track_name}-${t.artist}`} className="flex items-center justify-between">
                ...
                <span className="text-xs text-flowstate-500 shrink-0 ml-2">{t.count} 次</span>
              </div>
            ))}
          </div>
        </motion.div>
      ...
      {stats?.recentPlays?.length > 0 && (
        <motion.div
          ...
          variants={cardVariants}
        >
          <h3 className="text-sm font-medium text-flowstate-400 mb-3">最近播放</h3>
      ...
      <motion.div
        ...
        variants={cardVariants}
      >
        <h3 className="text-sm font-medium text-flowstate-400 mb-2">音乐品味</h3>
        <p className="text-sm text-white/60 whitespace-pre-line leading-relaxed">
          {taste?.taste ? taste.taste.slice(0, 300) : '暂无品味数据，和 FlowState 多聊聊吧'}
        </p>
      </motion.div>
      ...
      {taste?.playlists?.imported?.length > 0 && (
        <motion.div
          ...
          variants={cardVariants}
        >
          <h3 className="text-sm font-medium text-flowstate-400 mb-2">
          ...
          <div className="space-y-2">
            {taste.playlists.imported.map((pl) => (
              <div key={pl.id} className="flex items-center gap-2 text-sm text-white/60">
                <svg width="14" height="14" viewBox="0 0 24 24" fill="currentColor" className="text-flowstate-500 shrink-0">
