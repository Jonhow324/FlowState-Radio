...
四层去重在 AI 推荐歌曲**进入 NCM 解析前**执行（`chat.js` 和 `scheduler.js` 的 `autoRefillQueue`），通过 `state.getQueue()` 获取队列 ID（L2），通过 `state.getRecentPlaysForDedup(50)` 从 SQLite 查询播放历史（L3/L4）。被过滤的歌曲不会进入后续 NCM 解析和 Segment 生成流程。
...
- **自动 drain** — `enqueue()` 自动触发 drain 循环，无需手动调用
